...
状态：首个 CLI 版本已实现，`dual_issue_demo` 正在决策
...
Stage1 负责全局 Architecture、共享边界、项目级验证策略、实施顺序和用户批准。模块内部状态机、具体 Chisel 实现、完整测试开发和 baseline 集成进入 Stage2。性能优化进入 Stage3。

`STAGE1_COMPLETE` 表示：

1. 全局架构已经批准。
2. 工具链和项目骨架可以进入开发。
3. Stage2 的首批模块及依赖顺序已经确定。
...
| Module Map | 稳定 Module ID、职责、状态所有权、依赖关系和实施顺序 |
...
5. Module Map、Module ID、职责、依赖和实施顺序已经批准。
6. 跨模块协议与共享流水字段已经闭合。
